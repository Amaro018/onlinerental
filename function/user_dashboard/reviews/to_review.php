...
$tr = "SELECT * FROM tbl_transaction_log WHERE accNo = '$accNo' AND review_status = 0";
	...
	while($tr_row = mysqli_fetch_array($tr_query)){
		...
		$output .= '
			<tr>
				...
				<td '.$style.'>
					<button type="button" class="btn btn-outline-success btn-block btn-review" id="'.$tr_row['transaction_no'].'">Review</button>
				</td>
			</tr>
		';	
	}
