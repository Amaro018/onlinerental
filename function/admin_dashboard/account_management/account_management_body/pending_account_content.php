...
$status_table = $con->query("SELECT tbl_indreg.accNo, tbl_indreg.fname, tbl_indreg.lname, tbl_account.email,tbl_account.userType, tbl_status.pending FROM tbl_indreg INNER JOIN tbl_account ON tbl_indreg.accNo = tbl_account.accNo INNER JOIN tbl_status ON tbl_account.email = tbl_status.email WHERE pending = true AND userType = 'userShop' ");
		...
		<tr>
			...
			<td>
				<form mmethod="POST" action="view_pending_user_acc.php">
					...
					<a onclick="are you sure?, ok cancel" class="btn btn-primary text-light" href="pending_request.php?email='. $row['email'] .'">Accept</a>
